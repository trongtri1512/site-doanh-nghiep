...
const FujifilmImageDynamic = () => {
  ...
  const { data: brand } = useQuery({
    queryKey: ["brand", "fujifilm-image"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("brands")
        .select("*")
        .eq("slug", "fujifilm-image")
        .single();
        ...
        <div className="max-w-6xl mx-auto">
          {elements.length > 0 ? (
            ...
            <div className="text-center mb-12">
              <h1 className="text-4xl font-bold text-primary mb-4">Fujifilm Image Vietnam</h1>
              <p className="text-xl text-muted-foreground mb-8">
                Thương hiệu hình ảnh chuyên nghiệp từ Fujifilm
              </p>
